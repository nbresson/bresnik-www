...
|---|---|---|
| Marque | `Marque.astro` | Tuile du logo (`src/assets/marque/bresnik-logo.png`, marges transparentes rognées, 44 px de haut en en-tête (hauteur du bouton compact), 30 px en pied ; ajoutée le 2026-09-05) suivie du mot-symbole « Bresnik » en Bricolage 700, le « k » final en `cobalt`. Prop `taille` (`en-tete` 26 px, `pied` 22 px, mobile 24 px). Lien vers `/`. Le logo alimente aussi les favicons et les images de partage ; ses couleurs (bleu nuit, orange) ne sont pas reprises dans l'interface. |
...
| Pied de page | `Footer.astro` | Bordure haute `ligne`. Marque, mention « © 2026 Bresnik. Logiciels complémentaires pour l'écosystème Sage 100. Éditeur indépendant, sans lien avec Sage Group plc. », liens Mentions légales, Confidentialité, Flux RSS. L'année est fixée à 2026 (site statique). En mobile : empilé. Contient aussi les quatre liens principaux (Produits, Conseil, Blog, Contact) : c'est la navigation de repli quand JavaScript est désactivé. Depuis le 2026-09-05 : quatre colonnes (produits nommés, Bresnik, contact sans email, informations), puis marque, mention légale et « Retour en haut ». |
...
| Galerie | `Galerie.astro` | Grille de cadres de capture ; depuis le 2026-09-05, élément `bk-galerie` : chaque vignette ouvre une visionneuse (`<dialog>`, image WebP large, titre, position, précédent / suivant, flèches, Échap, focus rendu à la vignette) ; sans script, le lien ouvre la variante WebP dans un nouvel onglet. |
...
| Tuile de domaine | `TuileDomaine.astro` | Ajouté le 2026-09-05. Glyphe, titre, phrase facultative ; une, deux ou trois colonnes. Depuis le 2026-09-15 : aplat `papier-2` (ou `papier` avec `sur="papier-2"`) sans bordure ni survol, pour ne pas ressembler aux cartes cliquables. Domaines d'intervention (accueil, conseil) et fonctionnalités des fiches, où elle remplace la liste à coches. |
